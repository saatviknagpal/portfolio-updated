...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
  ...
  return (
    ...
    >
      {title}
      <span
        className={`h-[1px] inline-block  bg-light dark:bg-dark absolute w-0 left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 ${
          router.asPath === href ? "w-full" : "w-0"
        }         dark:bg-light`}
      >
        &nbsp;
        ...
        {isOpen ? (
          ...
          >
            ...
            <nav className="flex items-center justify-center flex-wrap ">
              ...
              <button
                className={`ml-5 sm:mx-1 flex items-center justify-center rounded-full p-1 w-7 ${
                  mode === "light" ? "bg-dark text-light" : "bg-light text-dark"
                }`}
                onClick={() => setMode(mode === "light" ? "dark" : "light")}
              >
